    ...
    news_finta = [
        {
            ...
            "topic": "Meteo",
            "luogo": "Napoli",
            "titolo": "Scuole chiuse a Napoli",
        }
    ]
    utente_finto = [(12345, "meteo", "napoli")]

    # Definisco i valori di ritorno per i vari moduli già testati
    with patch("news_bot.ricerca_notizia", return_value=news_finta), patch("news_bot.check_news", return_value=False), patch(
        "news_bot.clean_db"
    ) as mock_clean, patch("news_bot.execute_query", return_value=utente_finto):
